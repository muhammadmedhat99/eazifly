...
export const Sidebar = ({ onLinkClick }: { onLinkClick?: () => void }) => {
  ...
  const { data: perms = [], isLoading: loadingPerms } = useQuery({
    ...
    queryFn: () => fetchPermissions(token as string),
    enabled: !!token,
    staleTime: 1000 * 60 * 5, 
    gcTime: 1000 * 60 * 30, 
  });
  ...
  const { data: counts = {}, isLoading: loadingCounts } = useQuery({
    ...
    queryFn: () => fetchClient("client/count/data", axios_config),
    staleTime: 1000 * 60 * 2, 
    gcTime: 1000 * 60 * 10, 
  });
  ...
  const logout = useMutation({
    ...
    onSuccess: async (data) => {
      if (data.message !== "success") {
        ...
        document.cookie =
          "client_id=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
  ...
  const loading = loadingPerms || loadingCounts;
