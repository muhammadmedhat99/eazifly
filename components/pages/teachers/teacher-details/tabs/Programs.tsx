...
import { Avatar, AvatarGroup, CardBody, Chip, Dropdown, DropdownItem, DropdownMenu, DropdownTrigger, Tab, Table, TableBody, TableCell, TableColumn, TableHeader, TableRow, Tabs } from "@heroui/react";
...
export const Programs = () => {
    ...
    const fetchSessions = async (programId: number, page: number = 1) => {
        ...
        try {
            ...
            const formattedSessions = res?.data?.map((item: any) => ({
                id: item.id,
    ...
    const fetchStudents = async (programId: number, page: number = 1) => {
        ...
        try {
            ...
            const formattedStudents = res?.data?.map((item: any) => ({
                id: item.id,
                name: item.name,
                avatar: item.image,
                phone: item.phone,
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => await fetchClient(`client/instructor/programs/${instructor_id}`, axios_config),
    });
    ...
    const buildQueryParams = (programId, page = 1) => {
        ...
        <>
            {isLoading ? (
                ...
                <div className="p-4 grid grid-cols-1 gap-5">
                    ...
                    (<div key={index} className="p-4 bg-white rounded-lg border border-stroke">
                        <div className="p-5 bg-white rounded-xl outline outline-1 outline-blue-600">
                            <div className="flex flex-col gap-2">
                                ...
                                <span className="text-[#272727] text-sm font-bold">{program.title}</span>
                            </div>
                            <div className="py-4 grid grid-cols-2 md:grid-cols-3 gap-4">
                                ...
                                <div className="flex items-center justify-between bg-main p-5 rounded-xl border border-stroke">
                                    <div className="flex flex-col gap-4">
                                        <span className="text-[#5E5E5E] text-sm font-bold text-primary">سعر ساعة البرنامج</span>
                                        <span className="text-black-text font-bold text-[15px]">
                            ...
                            <div>
                                    ...
                                    <div className="p-2 bg-white rounded-lg border border-stroke">
                                        <div className="flex w-full flex-col">
                                            <Tabs
                                                ...
                                                selectedKey={tabsState[program.id]?.selectedTab || "program-lectures"}
                                                onSelectionChange={(key) => handleTabChange(program.id, key.toString())}
                                            >
                                                ...
                                                <Tab key="assignments" title={<span className="text-xs font-bold group-data-[selected=true]:text-white">التسليمات</span>} />
                                ...
                                <div className="mt-3">
                                    {tabsState[program.id]?.selectedTab === "program-lectures" && (
                                        tabsState[program.id]?.isSessionsLoading ? <Loader /> :
                                            <>
                                                <TableComponent
                                    ...
                                    {tabsState[program.id]?.selectedTab === "students" && (
                                        tabsState[program.id]?.isStudentsLoading ? <Loader /> :
                                            <>
                                                <TableComponent
                                                    columns={columns}
                                                ...
                                                <div className="my-10 px-6">
                                                    <CustomPagination
                                                        currentPage={currentPage}
                                                        setCurrentPage={setCurrentPage}
                                                        last_page={tabsState[program.id]?.metaStudents?.last_page}
                                    ...
                                    {tabsState[program.id]?.selectedTab  === "reports" && (
                                        tabsState[program.id]?.isReportsLoading ? <Loader /> : 
                                        <div>
                                            <Reports
                                                reportData={tabsState[program.id]?.reports}
                                                isLoadingReport={tabsState[program.id]?.isReportsLoading}
                                            />
                                        </div>
                                    )}
                                    {tabsState[program.id]?.selectedTab === "assignments" && (
                                        tabsState[program.id]?.isAssignmentsLoading ? <Loader /> :
                                            <div className="flex flex-col gap-2">
                                                {tabsState[program.id]?.assignments?.map((assignment, index) =>
                                                    <div
                                                        key={index}
                                                        ...
                                                        <div className="flex items-center gap-20 w-full">
                                                            <div className="flex items-center gap-20 w-1/2">
                                                                ...
                                                                <div className="flex flex-col gap-4 items-center w-1/3 ">
                                                                    ...
                                                                    <span className={`text-center text-sm font-bold ${assignment.status === "corrected"
                                                                        ? "text-[#0A9C71]"
                                                            ...
                                                            <div className="flex justify-end w-1/2">
                                                                <User
                                                                    avatarProps={{ radius: "full", src: assignment.user.image ?? undefined, size: "sm" }}
                                                                    description={
                                                                        <span className="text-sm font-bold text-[#3D5066]">{assignment.user.name}</span>
                                                                    }
                                                                    name={
                                                    ...
                                                    <CustomPagination
                                                        currentPage={currentPage}
                                                        setCurrentPage={setCurrentPage}
                                                        last_page={tabsState[program.id]?.metaAssignments?.last_page}
                                                        total={tabsState[program.id]?.metaAssignments?.total}
                                                    />
                                                </div>
                                            </div>
                                    )}
                                    {tabsState[program.id]?.selectedTab === "exams" && (
                                        tabsState[program.id]?.isExamsLoading ? <Loader /> :
                                            <div className="flex flex-col gap-2">
                                                            ...
                                                            <div className="flex items-center justify-between">
                                                                ...
                                                                <div className="flex flex-col items-center gap-3">
                                                                    <Chip
                                                                        ...
                                                                        size="sm"
                                                                    >
                                                                        <span className={`text-[#0A9C71] font-bold`}>
                                                                            {exam?.status}
                                                                        </span>
                                                                    </Chip>
                                                                    <span className="text-black-text text-sm font-bold">
                                                ...
                                                <div className="my-8 px-6">
                                                    <CustomPagination
                                                        currentPage={currentPage}
                                                        setCurrentPage={setCurrentPage}
                                                        last_page={tabsState[program.id]?.metaExams?.last_page}
