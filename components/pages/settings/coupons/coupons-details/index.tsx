...
export const CouponsDetails = ({ data }: CouponDetailsProps) => {  
    ...
    const { control, handleSubmit, watch } = useForm<FormData>({
        defaultValues: {
            ...
            times_used: data?.data?.times_used ? Number(data.data.times_used) : undefined,
        }
    });

    const { data: couponData, isLoading } = useQuery({
        queryKey: ['coupon', id],
        queryFn: async () => await fetchClient(`client/coupon/show/${id}`, axios_config),
    });
     ...
     return isLoading ? (
        <Loader />
             ...
             >
                     ...
                     <div className="flex flex-col gap-4">
                         ...
                         {editField === "discount" ? (
                             <Controller
                                 ...
                                 control={control}
                                 render={({ field }) => (
                                     <Input {...field} placeholder="قيمة الخصم" size="sm" type="number" />
                                 )}
                             />
                         ) : (
                             <span className="text-black-text font-bold text-[15px]">
                 ...
                 <div className="flex items-center justify-between bg-main p-5 rounded-2xl border border-stroke">
                     <div className="flex flex-col gap-4 w-1/2">
                         ...
                         {editField === "discount_type" ? (
                             ...
                             <span className="text-black-text font-bold text-[15px]">
                                 {couponData?.data?.discount_type === "fixed" ? "ثابت" : "نسبة مئوية"}
                             </span>
                         )}
                     </div>
                     {editField === "discount_type" ? (
                 ...
                 <div className="flex items-center justify-between bg-main p-5 rounded-2xl border border-stroke">
                     <div className="flex flex-col gap-4">
                         <span className="text-[#5E5E5E] text-sm font-bold">تاريخ الانتهاء</span>
                 ...
                 <div className="flex items-center justify-between bg-main p-5 rounded-2xl border border-stroke">
                     <div className="flex flex-col gap-4">
                         <span className="text-[#5E5E5E] text-sm font-bold">عدد مرات الاستخدام</span>
                             ...
                             <div className="flex flex-col md:flex-row gap-4 w-full">
                                 <div className="flex flex-col gap-2 w-full">
                                     <Controller
                                         ...
                                         control={control}
                                         render={({ field }) => (
                                             <Input {...field} placeholder="عدد مرات الاستخدام" size="sm" type="number" />
                                         )}
                                     />
                                 </div>
                             </div>
                 ...
                 <div className="flex items-center justify-between bg-main p-5 rounded-2xl border border-stroke">
                     <div className="flex flex-col gap-4">
                         <span className="text-[#5E5E5E] text-sm font-bold">تم استخدامه مسبقًا</span>
                         <span className="text-black-text font-bold text-[15px]">
