...
const columns = [
  { name: "", uid: "avatar" },
  { name: "إسم الدولة", uid: "name" },
  { name: "كود الدولة", uid: "country_code" },
  { name: "مفتاح الدولة", uid: "phone_code" },
...
export const AllCountries = () => {
  ...
  const { data: countriesData, isLoading } = useQuery({
    queryFn: async () =>
      await fetchClient(`client/countries`, {
        ...axios_config,
        params,
      }),
  ...
  const formattedData =
    countriesData?.data?.map((item: any) => ({
      id: item.id,
      avatar: item.image,
      name: item.name || "N/A",
      country_code: item.country_code,
      phone_code: item.phone_code || "N/A",
    ...
    <>
      ...
      <div className="my-10 px-6">
        <CustomPagination
          ...
          last_page={countriesData?.meta?.last_page}
          total={countriesData?.meta?.total}
        />
      </div>
    </>
  );
