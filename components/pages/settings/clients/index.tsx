...
export const AllClients = () => {
  ...
  const { data: clientsData, isLoading } = useQuery({
    queryFn: async () =>
      await fetchClient(`client`, {
        ...axios_config,
        params,
      }),
    queryKey: AllQueryKeys.GetAllClients
  });
  ...
  const formattedData =
    clientsData?.data?.map((item: any) => ({
      ...
      roles: item.roles[0] || 'null',
      created_at: formatDate(item.created_at) || 'null',
    })) || [];
    ...
    <>
      ...
      <div className="my-10 px-6">
        <CustomPagination
          ...
          last_page={clientsData?.meta?.last_page}
          total={clientsData?.meta?.total}
        />
      </div>
    </>
  );
