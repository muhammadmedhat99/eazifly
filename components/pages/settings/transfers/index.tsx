...
export const AllTransfers = () => {
  ...
  const { data: transfersData, isLoading } = useQuery({
    queryFn: async () =>
      await fetchClient(`client/instructor/payment/method`, {
        ...axios_config,
        params,
      }),
  ...
  const formattedData =
    transfersData?.data?.map((item: any, idx: number) => ({
      id: item.id,
      index: idx + 1,
    ...
    <>
      ...
      <div className="my-10 px-6">
        <CustomPagination
          ...
          last_page={transfersData?.meta?.last_page}
          total={transfersData?.meta?.total}
        />
      </div>
    </>
  );
