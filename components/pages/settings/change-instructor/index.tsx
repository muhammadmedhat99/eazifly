  ...
  return (
    <Dropdown classNames={{ base: "max-w-40", content: "min-w-36" }}>
      ...
      <DropdownMenu aria-label="Static Actions">
        <DropdownItem href={`/settings/change-instructor/${id}`} key="show">
          عرض البيانات
        </DropdownItem>
      </DropdownMenu>
    </Dropdown>
...
export const AllChangeInstructorReasons = () => {
  ...
  const { data: changeInstructorReasons, isLoading } = useQuery({
    queryFn: async () =>
      await fetchClient(`client/reason/change/instructor`, {
        ...axios_config,
        params,
      }),
  ...
  const formattedData =
    changeInstructorReasons?.data?.map((item: any) => ({
      id: item.id,
      index: item.id,
      name: item.title || "N/A",
    ...
    <>
      ...
      <div className="my-10 px-6">
        <CustomPagination
          ...
          last_page={changeInstructorReasons?.meta?.last_page}
          total={changeInstructorReasons?.meta?.total}
        />
      </div>
    </>
  );
