...
export const CreateChangeInstructorReason = () => {
    const {
        register,
        handleSubmit,
        formState: { errors },
    ...
    const CreateReason = useMutation({
        mutationFn: (submitData: FormData) => {
            ...
            return postData("client/reason/change/instructor/store", formdata, myHeaders);
        },
        onSuccess: (data) => {
            if (data.message && typeof data.message === "object" && !Array.isArray(data.message)) {
                ...
                router.push('/settings/change-instructor')
            }
        },
        onError: (error) => {
