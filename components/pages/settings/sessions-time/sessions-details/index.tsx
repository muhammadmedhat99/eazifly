...
export const SessionsDetails = ({ data }: SessionDetailsProps) => {
    ...
    const { data: sessionData, isLoading } = useQuery({
        queryKey: ['sessionTime', id],
        queryFn: async () => await fetchClient(`client/plan/session/time/show/${id}`, axios_config),
    });

    const onSubmit = (data: FormData) => UpdateSession.mutate(data);
    ...
    const UpdateSession = useMutation({
        mutationFn: (submitData: FormData) => {
            ...
            return postData(
                `client/plan/session/time/update/${data.data.id}`,
                formdata,
                myHeaders
            );
        },
        onSuccess: (data) => {
            if (data.message !== "success") {
                ...
                queryClient.invalidateQueries({ queryKey: ['sessionTime', id] });
            }
            setEditField(null);
        },
        onError: (error) => {
        ...
        >
                ...
                <div className="flex flex-col gap-4">
                    ...
                    {editField === "name" ? (
                        ...
                        <span className="text-black-text font-bold text-[15px]">
                            {`${sessionData?.data?.title_ar} / ${sessionData?.data?.title_en}`}
                        </span>
                    )}
                </div>
            ...
            <div className="flex items-center justify-between bg-main p-5 rounded-2xl border border-stroke">
                <div className="flex flex-col gap-4">
                    ...
                    <span className="text-black-text font-bold text-[15px]">
                        {formatDate(sessionData?.data?.created_at)}
                    </span>
                </div>
            </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-4 w-full">
                            <div className="flex flex-col gap-2 w-full">
                                <Controller
                                    ...
                                    control={control}
                                    render={({ field }) => (
                                        <Input {...field} placeholder="مدة المحاضرة" size="sm" />
                                    )}
                                />
                            </div>
                        </div>
