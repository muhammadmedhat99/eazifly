...
export const Permissions = () => {
  ...
  const { data: permissionsData, isLoading: isPermissionsLoading } = useQuery({
    queryFn: async () => await fetchClient(`client/get/permission`, axios_config),
      ...
      <div className="flex flex-col relative touch-none tap-highlight-transparent select-none w-full bg-content1 border border-stroke max-w-full justify-between rounded-2xl gap-5 p-4 data-[selected=true]:border-success">
        <div className="flex flex-col gap-2">
          <div className="inline-flex justify-start items-center gap-1">
            <span className="text-[#272727] text-sm font-bold">إسم الوظيفة</span>
            <span className="text-primary text-xs font-bold">( 42 صلاحية )</span>
          </div>
          <Controller
            name="role"
            control={control}
            render={({ field }) => (
              ...
              >
                {(rolesData?.data as { id: number; name: string }[])?.map((role) => (
