...
const schema = yup
    .object({
        name_ar: yup
            .string()
            .required("ادخل الرد بالعربية")
            .min(3, "الرد لا يجب أن يقل عن ٣ أحرف"),

        name_en: yup
            .string()
            .required("ادخل الرد بالإنجليزية")
            .min(3, "الرد لا يجب أن يقل عن ٣ أحرف"),
    })
    .required();
...
export const CreateResonses = () => {
    ...
    const onSubmit = (data: FormData) => CreateResponse.mutate(data);
    ...
    const CreateResponse = useMutation({
        mutationFn: (submitData: FormData) => {
            ...
            return postData("client/user/response/store", formdata, myHeaders);
        },
        onSuccess: (data) => {
            if (data.message && typeof data.message === "object" && !Array.isArray(data.message)) {
                ...
                router.push('/settings/user-responses')
            }
        },
        onError: (error) => {
        ...
        >
            ...
            <div className="flex items-center justify-end gap-4 mt-8 col-span-2">
                ...
                <Button
                    ...
                    color="primary"
                    className="text-white"
                    isDisabled={CreateResponse?.isPending}
                >
                    {CreateResponse?.isPending && <Spinner color="white" size="sm" />}
                    التالي
                </Button>
            </div>
        </form>
