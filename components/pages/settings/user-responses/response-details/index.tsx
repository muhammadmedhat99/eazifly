...
export const ResponseDetails = ({ data }: SpecializationDetailsProps) => {
    ...
    const { data: responseData, isLoading } = useQuery({
        ...
        queryFn: async () => await fetchClient(`client/user/response/show/${id}`, axios_config),
    });

    const onSubmit = (data: FormData) => UpdateResponse.mutate(data);
    ...
    const UpdateResponse = useMutation({
        mutationFn: (submitData: FormData) => {
            ...
            return postData(
                `client/user/response/update/${data.data.id}`,
                formdata,
                myHeaders
            );
        },
        ...
        >
                ...
                <div className="flex flex-col gap-4">
                    ...
                    {editField === "name" ? (
                        ...
                        <span className="text-black-text font-bold text-[15px]">
                            {`${responseData?.data?.title_ar} / ${responseData?.data?.title_en}`}
                        </span>
                    )}
                </div>
            ...
            <div className="flex items-center justify-between bg-main p-5 rounded-2xl border border-stroke">
                <div className="flex flex-col gap-4">
                    ...
                    <span className="text-black-text font-bold text-[15px]">
                        {formatDate(responseData?.data?.created_at)}
                    </span>
                </div>
            </div>
